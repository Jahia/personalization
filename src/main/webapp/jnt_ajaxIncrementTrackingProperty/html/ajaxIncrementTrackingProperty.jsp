...
<c:choose>
    <c:when test="${renderContext.editMode}">
        <div class="personalizationbox">
            <h4><fmt:message key="label.ajaxIncrementTrackingProperty"/></h4>
            <p><fmt:message key="label.ajaxIncrementTrackingProperty.componentDescription"/></p>
            <fmt:message key="label.ajaxIncrementTrackingProperty.willIncrementProperty">
    ...
    <c:otherwise>
        <script type="text/javascript">
            $(document).ready(function() {
                $.post('<c:url value="/cms/tracking"/>', { <c:out value="${propertyName.string}"/> : '+=1' });
            });
        </script>
    </c:otherwise>
</c:choose>